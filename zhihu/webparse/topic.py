...
from zhihu.base.network import get_request, post_request
from zhihu.base.network import login, get_url_id, ZhiHuPage
from zhihu.base import is_num_by_except
from zhihu.setting import ZHI_HU_URL
import question

session = login(email='1054059790@qq.com', passwd='xiao105405')
...
class TopicNode(object):
    def __init__(self, url):
        self.soup = BeautifulSoup(get_request(session, url).content)
        self.url = url
    ...
    def get_children_nodes(self):
        ...
        while True:
            ...
            response = post_request(session, post_url, data=data)
            msg = response.json()["msg"]
            topic_list = msg[1]

            for item in topic_list:
                if item[0][0] == "topic":
                    url = "http://www.zhihu.com/topic/{0}/organize/entire".format(item[0][2])
                    yield TopicNode(url)
                elif item[0][0] == "load":
                    data_token = item[0][2]
            ...
            if data_token == None and data_parent == None:
                return
            post_url = "{0}?child={1}&parent={2}".format(self.url, data_token, data_parent)
